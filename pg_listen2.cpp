...
void increase_ragea()
{
     ...
     */
    res = PQexec(conn, "LISTEN notify_customcheckinserted");
...
void increase_rageb()
{
     ...
     * Issue LISTEN command to enable notifications from the rule's NOTIFY.
     */
    res = PQexec(conn, "LISTEN notify_customcheckdeleted");
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
    {
        fprintf(stderr, "LISTEN command failed: %s", PQerrorMessage(conn));
        PQclear(res);
...
void increase_ragec()
{
     ...
     * Issue LISTEN command to enable notifications from the rule's NOTIFY.
     */
    res = PQexec(conn, "LISTEN notify_custommancheckinserted");
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
    {
        fprintf(stderr, "LISTEN command failed: %s", PQerrorMessage(conn));
        PQclear(res);
...
void increase_raged()
{
     ...
     * Issue LISTEN command to enable notifications from the rule's NOTIFY.
     */
    res = PQexec(conn, "LISTEN notify_custommancheckdeleted");
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
    {
        fprintf(stderr, "LISTEN command failed: %s", PQerrorMessage(conn));
        PQclear(res);
...
main(int argc, char **argv)
{
    ...
    std::thread a(increase_ragea), b(increase_rageb), c(increase_ragec), d(increase_raged);
